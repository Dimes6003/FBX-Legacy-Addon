...
bl_info = {
    "name": "FBX Legacy Formats",
    "author": "Campbell Barton, Bastien Montagne, Jens Restemeier",
    "version": (4, 29, 1),
    "blender": (2, 90, 0),
    "location": "File -> Import-Export -> FBX Legacy (.fbx)",
...
if "bpy" in locals():
    import importlib
    import sys
    if ("import_fbx" in locals() and "io_scene_fbx_legacy.import_fbx" in sys.modules):
        importlib.reload(import_fbx)
    if ("export_fbx_bin" in locals() and "io_scene_fbx_legacy.export_fbx_bin" in sys.modules):
        importlib.reload(export_fbx_bin)
    if ("export_fbx" in locals() and "io_scene_fbx_legacy.export_fbx" in sys.modules):
...
class FBXAddonPreferences(bpy.types.AddonPreferences):
    bl_idname = __package__

    fbx_version: EnumProperty(
            name="FBX Legacy Version",
            description="FBX version to use for FBX Legacy exporting",
            items=(
                ('200611', "2006 (Maya 6.0 - 8.5)", ""),
                ('200900', "2009 (Maya 2009)", ""),
                ('201000', "2010 (Maya 2010)", ""),
                ('201100', "2011 (Maya 2011)", ""),
                ('201200', "2012 (Maya 2012)", ""),
                ('201300', "2013 (Maya 2013)", ""),
            ),
            default='200611',
            )
            
    use_maya_export_defaults: BoolProperty(
            name="Use Maya Specialized Export Defaults",
            description="Assigns export default settings oriented towards Maya ↔ Blender workflows",
            default=True,
            )

    fbx_converter_path: StringProperty(
            name="FBX Legacy Converter",
            description="Path to FbxConverter.exe used for FBX Legacy exporting",
            subtype='FILE_PATH',
            default="",
            )
    ...
    def draw(self, context):
        ...
        # FBX Version
        box = layout.box()
        box.label(text="FBX Version", icon='TOOL_SETTINGS')

        row = box.row(align=True)
        row.prop(self, "fbx_version", expand=False)
        
        # Maya Directed Defaults
        box.prop(self, "use_maya_export_defaults")

        # FBX Converter
        box = layout.box()
        box.label(text="FBX Converter", icon='FILE')

        col = box.column(align=True)
        col.prop(self, "fbx_converter_path", text="FbxConverter.exe")


@orientation_helper(axis_forward='-Z', axis_up='Y')
class ImportFBXProperties:
...
def import_fbx_file(self, context):
    import importlib

    keywords = self.as_keywords(ignore=("filter_glob", "directory", "ui_tab", "filepath", "files"))
    import_fbx_module = importlib.import_module(".import_fbx", package=__package__)

    return import_fbx_module.load(self, context, filepath=self.filepath, **keywords)


def convert_fbx_import(self, converter_path, source_filepath, destination_filepath):
    import subprocess
    ...
    command = [
        ...
        "/sffFBX",
        "/dffFBX",
        "/f201300" # latest version (2013)
    ]
    
    print("[FBX Converter] sending command: {}".format(subprocess.list2cmdline(command)))

    try:
        subprocess.run(command, check=True)
    except subprocess.CalledProcessError as e:
        self.report({'ERROR'}, "FbxConverter.exe failed with exit code {}.".format(e.returncode))
        return False

    except Exception as e:
        self.report({'ERROR'}, "Failed to run FbxConverter.exe: {}".format(e))
        return False
...
@orientation_helper(axis_forward='-Z', axis_up='Y')
class ImportFBXLegacy(ImportFBXProperties, bpy.types.Operator, ImportHelper):
    """Load a FBX file"""
    bl_idname = "import_scene.fbx_legacy"
    bl_label = "Import FBX Legacy"
    ...
    def execute(self, context):
        ...
        # Get converter preferences.
        preferences = context.preferences.addons[__package__].preferences
        converter_path = preferences.fbx_converter_path

        # validate the converter path
        if (not converter_path or not os.path.isfile(converter_path) or not converter_path.lower().endswith(".exe")):
            self.report(
                {'ERROR'},
                "FbxConverter.exe not found: {}".format(
                    converter_path or "<not configured>"
                )
            )
            return {'CANCELLED'}

        # remember the user's selected path
        selected_directory = os.path.dirname(self.filepath)
        selected_filename = os.path.basename(self.filepath)
        selected_file = self.filepath

        print("[FBX Legacy] User selected file: {}".format(selected_file))

        # define the temp directories
        temp_directory = os.path.join(bpy.app.tempdir, "io_scene_fbx_legacy")
        os.makedirs(temp_directory, exist_ok=True)

        temp_filename = "{}.fbx".format(os.getpid())
        temp_filepath = os.path.join(temp_directory, temp_filename) 

        print("[FBX Legacy] Temporary FBX path: {}".format(temp_filepath))

        # convert the selected FBX to parser compatible version
        result = convert_fbx_import(self, converter_path, selected_file, temp_filepath)
        if not result:
            return {'CANCELLED'}

        # assign temp filepath as import path
        self.filepath = temp_filepath

        # Import the FBX.
        result = import_fbx_file(self, context)
        self.filepath = selected_file # restore the selected filepath
        if result != {'FINISHED'}:
            return result

        self.report({'INFO'}, "FBX Import Finished")
        return {'FINISHED'}
...
def export_fbx_file(self, context, selected_file):
    ...
    keywords["selected_file"] = selected_file

    export_fbx_bin_module = importlib.import_module(".export_fbx_bin", package=__package__)

    return export_fbx_bin_module.save(self, context, **keywords)


def convert_fbx_export(self, converter_path, source_filepath, destination_filepath, fbx_version):
    import subprocess
...
@orientation_helper(axis_forward='-Z', axis_up='Y')
class ExportFBXLegacy(ExportFBXProperties, bpy.types.Operator, ExportHelper):
    """Write a FBX Legacy file."""
    bl_idname = "export_scene.fbx_legacy"
    bl_label = "Export FBX Legacy"
    bl_options = {'UNDO', 'PRESET'}
    ...
    def execute(self, context):
        ...
        # assign temp filepath as export path
        self.filepath = temp_filepath

        # generate the normal FBX
        result = export_fbx_file(self, context, selected_file)
        self.filepath = selected_file # restore the selected filepath
        if result != {'FINISHED'}:
            return result

        # convert the temporary FBX to the requested legacy version
        result = convert_fbx_export(self, converter_path, temp_filepath, selected_file, preferences.fbx_version)
        if not result:
            return {'CANCELLED'}
...
def menu_func_import_legacy(self, context):
    self.layout.operator(ImportFBXLegacy.bl_idname, text="FBX Legacy (.fbx)")

def menu_func_export_legacy(self, context):
    self.layout.operator(ExportFBXLegacy.bl_idname, text="FBX Legacy (.fbx)")
...
def register():
    ...
    if not preferences.fbx_converter_path:
        bundled_converter = preferences.get_bundled_converter_path()

        if bundled_converter:
            preferences.fbx_converter_path = bundled_converter

    bpy.types.TOPBAR_MT_file_import.append(menu_func_import_legacy)
...
def delete_temp_files():
    ...
    # remove temp file data
    temp_directory = os.path.join(bpy.app.tempdir, "io_scene_fbx_legacy")
    pid = str(os.getpid())
    try:
        if os.path.isdir(temp_directory):
            for entry in os.listdir(temp_directory):
                if not entry.startswith(pid + "."):
                    continue

                entry_path = os.path.join(temp_directory, entry)
                try:
                    if not os.path.isdir(entry_path):
                        print("[FBX Legacy] Removing temporary file: {}".format(entry_path))
                        os.remove(entry_path)

                except Exception as e:
                    print(
                        "[FBX Legacy] Could not delete temporary file at {}\n"
                        "Error: {}".format(entry_path, e)
                    )

    except Exception as e:
        print(
            "[FBX Legacy] Could not access temporary directory at {}\n"
            "Error: {}".format(temp_directory, e)
        )
...
def unregister():
    ...
    bpy.types.TOPBAR_MT_file_import.remove(menu_func_import_legacy)
