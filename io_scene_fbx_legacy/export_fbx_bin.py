...
def save_single(operator, scene, depsgraph, filepath="", selected_file="",
    ...
    media_settings = FBXExportSettingsMedia(
        ...
        os.path.dirname(selected_file),  # base_dst
        # Local dir where to put images (media), using FBX conventions.
        os.path.splitext(os.path.basename(selected_file))[0] + ".fbm",  # subdir
        embed_textures,
        set(),  # copy_set
        set(),  # embedded_set
    )
...
def save(operator, context,
    ...
    if batch_mode == 'OFF':
        ...
        ret = save_single(operator, context.scene, depsgraph, filepath, selected_file=selected_file, **kwargs_mod)
    else:
        ...
        for data, data_name, data_obj_propname in data_seq:  # scene or collection
            ...
            save_single(operator, scene, scene.view_layers[0].depsgraph, filepath, selected_file=selected_file, **kwargs_batch)
